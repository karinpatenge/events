...
DECLARE
  ...
  c_item_count CONSTANT PLS_INTEGER := 44000;
...
BEGIN
  ...
  IF v_header_count != c_bom_count THEN
    RAISE_APPLICATION_ERROR(
      -20014,
      'BOM_HEADERS verification failed. Expected ' || c_bom_count || ' rows; found ' || v_header_count || '.'
    );
  END IF;
  ...
  IF v_item_count != c_item_count THEN
    RAISE_APPLICATION_ERROR(
      -20015,
      'BOM_ITEMS verification failed. Expected ' || c_item_count || ' rows; found ' || v_item_count || '.'
    );
  END IF;

  DBMS_OUTPUT.PUT_LINE('BOM_HEADERS verified. Rows loaded: ' || v_header_count);
  DBMS_OUTPUT.PUT_LINE('BOM_ITEMS verified. Rows loaded: ' || v_item_count);
END;
/


PROMPT === BOM_HEADERS and BOM_ITEMS loaded ===