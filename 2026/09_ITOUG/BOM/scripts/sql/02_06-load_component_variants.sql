...
BEGIN
  ...
  IF v_row_count != c_variant_count THEN
    RAISE_APPLICATION_ERROR(
      -20013,
      'BOM_COMPONENT_VARIANTS verification failed. Expected ' || c_variant_count || ' rows; found ' || v_row_count || '.'
    );
  END IF;

  DBMS_OUTPUT.PUT_LINE('BOM_COMPONENT_VARIANTS verified. Rows loaded: ' || v_row_count);
END;
/

PROMPT === BOM_COMPONENT_VARIANTS loaded ===