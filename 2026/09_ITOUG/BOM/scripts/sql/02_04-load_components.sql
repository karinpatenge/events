...
BEGIN
  ...
  IF g_cat_count = 0 THEN
    RAISE_APPLICATION_ERROR(-20001, 'No BOM_CATEGORIES rows found. Run 02_03-load_categories.sql first.');
  END IF;
  ...
  IF v_row_count != c_component_count THEN
    RAISE_APPLICATION_ERROR(
      -20011,
      'BOM_COMPONENTS verification failed. Expected ' || c_component_count || ' rows; found ' || v_row_count || '.'
    );
  END IF;

  DBMS_OUTPUT.PUT_LINE('BOM_COMPONENTS verified. Rows loaded: ' || v_row_count);
END;
/

PROMPT === BOM_COMPONENTS loaded ===