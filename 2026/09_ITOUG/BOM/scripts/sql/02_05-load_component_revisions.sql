...
DECLARE
  c_revision_count CONSTANT PLS_INTEGER := 17500;
...
BEGIN
  ...
  IF v_row_count != c_revision_count THEN
    RAISE_APPLICATION_ERROR(
      -20012,
      'BOM_COMPONENT_REVISIONS verification failed. Expected ' || c_revision_count || ' rows; found ' || v_row_count || '.'
    );
  END IF;

  DBMS_OUTPUT.PUT_LINE('BOM_COMPONENT_REVISIONS verified. Rows loaded: ' || v_row_count);
END;
/

PROMPT === BOM_COMPONENT_REVISIONS loaded ===