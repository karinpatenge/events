...
PROMPT
PROMPT === Verifying BOM_CATEGORIES row count ===
...
BEGIN
  ...
  IF v_row_count != 27 THEN
    RAISE_APPLICATION_ERROR(
      -20010,
      'BOM_CATEGORIES verification failed. Expected 27 rows; found ' || v_row_count || '.'
    );
  END IF;

  DBMS_OUTPUT.PUT_LINE('BOM_CATEGORIES verified. Rows loaded: ' || v_row_count);
END;
/
