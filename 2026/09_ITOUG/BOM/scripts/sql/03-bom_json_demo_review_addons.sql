--==============================================================================
--  02_09-bom_json_demo_review_addons.sql
...
WHERE
  JSON_VALUE(c.compliance, '$.regulatory.functionalSafetyLevel' RETURNING VARCHAR2(20)) IN ('ASIL-C', 'ASIL-D')
ORDER BY
  carbon_kg DESC
...
FROM
  bom_headers h
WHERE
  JSON_EXISTS(h.bom_metadata, '$.commercialTargets.targetMarkets[*]?(@ == "EU")')
  AND h.bom_status = 'RELEASED'
ORDER BY
  h.model_year, h.vehicle_model
FETCH FIRST 20 ROWS ONLY;
...
GROUP BY
  jt.market_code, h.powertrain_type
ORDER BY
  jt.market_code, h.powertrain_type;
...
SELECT
  LPAD(' ', (bi.level_num - 1) * 2) || bi.find_number || '  ' || c.part_number || '  ' || c.component_name AS tree_line,
  bi.quantity,
  bi.level_num,
...
WHERE
  bi.bom_id = (SELECT MIN(bom_id) FROM bom_headers WHERE bom_status = 'RELEASED')
ORDER BY
  bi.sequence_num;
...
PROMPT -- SET
PROMPT --   compliance = JSON_TRANSFORM(
PROMPT --     compliance,
PROMPT --      SET '$.audit.lastReviewedBy' = 'bom-demo',
PROMPT --      SET '$.audit.reviewedAt' = SYSTIMESTAMP FORMAT JSON
PROMPT --   )
PROMPT -- WHERE
PROMPT --   component_id = (SELECT MIN(component_id) FROM bom_components);
