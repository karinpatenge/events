--==============================================================================
--  03_03b_create_bom_property_graph_using_materialized_views.sql
...
--  <original_table_name>_mv views for tables that contain JSON columns.
--==============================================================================
...
CREATE PROPERTY GRAPH IF NOT EXISTS bom_graph_expanded
  VERTEX TABLES (
    ...
    bom_components_mv AS bom_components
      KEY (component_id)
      LABEL component
      PROPERTIES ARE ALL COLUMNS,
    bom_component_revisions_mv AS bom_component_revisions
      KEY (revision_id)
      LABEL component_revision
      PROPERTIES ARE ALL COLUMNS,
    bom_component_variants_mv AS bom_component_variants
      KEY (variant_id)
      LABEL component_variant
      PROPERTIES ARE ALL COLUMNS,
    bom_headers_mv AS bom_headers
      KEY (bom_id)
      LABEL header
      PROPERTIES ARE ALL COLUMNS,
    bom_items_mv AS bom_items
      KEY (bom_item_id)
      LABEL item
      PROPERTIES ARE ALL COLUMNS
  )
  EDGE TABLES (
    bom_items_mv AS bom_items_bom_items
      KEY (bom_item_id)
      SOURCE KEY (parent_item_id) REFERENCES bom_items (bom_item_id)
      DESTINATION KEY (bom_item_id) REFERENCES bom_items (bom_item_id)
      LABEL contains_item
      NO PROPERTIES,
    bom_items_mv AS bom_items_bom_headers
      KEY (bom_item_id)
      SOURCE KEY (bom_id) REFERENCES bom_headers (bom_id)
      DESTINATION KEY (bom_item_id) REFERENCES bom_items (bom_item_id)
      LABEL has_item
      NO PROPERTIES,
    bom_items_mv AS bom_items_bom_component_variants
      KEY (bom_item_id)
    ...
    bom_components_mv AS bom_components_bom_categories
      ...
      LABEL in_category
      NO PROPERTIES,
    bom_component_revisions_mv AS bom_components_bom_components_revisions
      KEY (revision_id)
      SOURCE KEY (component_id) REFERENCES bom_components (component_id)
      DESTINATION KEY (revision_id) REFERENCES bom_component_revisions (revision_id)
      LABEL has_revision
      NO PROPERTIES,
    bom_component_variants_mv AS bom_components_bom_component_variants
      KEY (variant_id)
      SOURCE KEY (component_id) REFERENCES bom_components (component_id)
      DESTINATION KEY (variant_id) REFERENCES bom_component_variants (variant_id)
      LABEL has_variant
      NO PROPERTIES,
    bom_headers_mv AS bom_headers_bom_components
      KEY (bom_id)
      SOURCE KEY (bom_id) REFERENCES bom_headers (bom_id)
