--==============================================================================
--  03_02a_create_json_projection_views.sql
--  First-level JSON projection views for the BOM demo schema
--  Target: Oracle AI Database 26ai
--
--  JSON handling:
--    * Tables with JSON columns are exposed through views named:
--        <original_table_name>_vw
--    * Each view is generated from Oracle JSON Data Guide metadata.
...
PROMPT
PROMPT === Creating first-level JSON projection views from Data Guide output ===
...
DECLARE
    ...
    PROCEDURE create_json_projection_view(
    ...
    BEGIN
        ...
        append_text(l_sql, 'CREATE OR REPLACE VIEW ' || p_view_name || ' AS' || CHR(10));
        ...
        DBMS_OUTPUT.PUT_LINE('Created ' || p_view_name || ' with first-level JSON projections.');
        DBMS_LOB.FREETEMPORARY(l_select_list);
        DBMS_LOB.FREETEMPORARY(l_sql);
    END create_json_projection_view;
BEGIN
    create_json_projection_view(
        p_view_name    => 'bom_components_vw',
    ...
    create_json_projection_view(
        p_view_name    => 'bom_component_revisions_vw',
    ...
    create_json_projection_view(
        p_view_name    => 'bom_component_variants_vw',
...
PROMPT
PROMPT === JSON projection views created ===

SELECT object_name, object_type, status
FROM   user_objects
WHERE  object_name IN (
           'BOM_COMPONENTS_VW',
           'BOM_COMPONENT_REVISIONS_VW',
           'BOM_COMPONENT_VARIANTS_VW',
           'BOM_HEADERS_VW',
           'BOM_ITEMS_VW'
       )
...
PROMPT
PROMPT === Generated first-level JSON projection columns ===
